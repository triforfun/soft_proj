...
function get_available_bikes($size, $start_date, $end_date) {
    ...
    foreach ($filtered_bikes as &$bike) {
        ...
        foreach ($reservations as $reservation) {
            // Solo restar stock si la reserva solapa con las fechas solicitadas
            if (
                $reservation['bike_id'] == $bike['id'] &&
                // Verificar si las fechas de la reserva se solapan con las solicitadas
                (strtotime($reservation['start_date']) < strtotime($end_date) && strtotime($reservation['end_date']) > strtotime($start_date))
            ) {
                $bike_stock--; // Decrementar el stock si hay un solapamiento
            }
        }
...
// Actualizar el estado de la bicicleta a no disponible solo si es realmente necesario
$bikes_file = get_stylesheet_directory() . '/bikes.json';
$bikes = json_decode(file_get_contents($bikes_file), true);

// Encontrar la bicicleta y marcarla como no disponible para el rango de fechas
foreach ($bikes as &$bike) {
    if ($bike['id'] == $bike_id) {
        // Solo la marcamos como no disponible si hay reservas solapadas
        $bike['available'] = false; // Marcar como no disponible
        $bike['stock']--; // Decrementar el stock
        break; // Salir del bucle después de encontrar la bicicleta
    }
}
// Guardar los cambios en bikes.json
file_put_contents($bikes_file, json_encode($bikes, JSON_PRETTY_PRINT));