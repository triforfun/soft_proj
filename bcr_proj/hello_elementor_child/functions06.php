...
function save_reservation($bike_id, $start_date, $end_date) {
    ...
    // Ahora actualizar la disponibilidad de la bicicleta
    update_bike_availability($bike_id);
}

function update_bike_availability($bike_id) {
    $bikes_file = get_stylesheet_directory() . '/bikes.json';
    $bikes = json_decode(file_get_contents($bikes_file), true);

    foreach ($bikes as &$bike) {
        if ($bike['id'] == $bike_id) {
            $bike['available'] = false; // Marcar la bicicleta como no disponible
            $bike['stock']--; // Reducir el stock
            break;
        }
    }
